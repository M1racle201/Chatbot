...
- 检索原文可能只是文档片段,不一定覆盖完整岗位记录;检索片段不完整时,不要直接判定回答缺失
- 如果回答已经列出了用户明确要求的字段,且这些字段之间没有与原文冲突,即使当前检索片段没有展示全部字段,也不要因为证据片段不完整而打回
- 原文中暂时找不到依据的断言,只有回答与检索原文冲突、导致核心结论不可靠,或回答确实遗漏用户明确要求的核心内容时,才判定不合格
- 答案核心观点正确、相关且能够直接满足用户意图,即使存在可以补充的信息 → 视为合格
- 存在明显事实错误,严重遗漏导致无法回答问题,与用户问题偏离,或缺少必要证据 → 视为不合格 → 在suggestion填入意见
- 若未提供检索原文,则凭完整性与逻辑一致性判断

不通过时,判断打回类型(action 字段):
- "rewrite":复写或表述不佳导致的理解偏差,重新表述即可解决
- "research":只有当检索原文与结论冲突,或当前证据不足以支持核心结论时才使用;不能仅因检索片段未覆盖全部字段就使用 research
- 通过时 action 为 "pass"
